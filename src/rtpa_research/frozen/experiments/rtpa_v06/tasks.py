...
def tests(tokenizer):
    ...
    for text,expected in [('4827','4827'),(' 4827 ','4827'),('04827',None),('4828','4828'),('',None),('value 4827',None),('4827 9123',None),('4827 extra',None),('4827\n','4827')]:
        ck('parser_'+repr(text),parse_answer(text)['answer']==expected)
    ck('wrong_answer_not_substring',parse_answer('The answer may be 4827 or 9123')['answer'] is None)
    for template in (1,2):
        rows=panel(tokenizer,'CAL'+str(template),4,template)
        ck('GT_and_length_template'+str(template),all(independent_gt(r)==r['ground_truth'] and LENGTHS[r['length']][0]<=len(r['token_ids'])<=LENGTHS[r['length']][1] for r in rows))
        ck('deterministic_template'+str(template),rows==panel(tokenizer,'CAL'+str(template),4,template))
        save(ART/f'cal_panel_template{template}.json',{'items':rows,'role':'Native-only capability; no quantized accuracy selection','template':template})
    return checks
