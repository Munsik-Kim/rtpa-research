...
class R2PayloadLayer(LinearAttentionLayer):
    ...
    def recurrent_states(self):
        ...
        try:
            ...
            self.last_failure={'boundary':'payload_decode_before_state_use','write_index':self.write_count,
                               'error':str(exc),'payload':{k:v.detach().cpu() for k,v in self.payload.items()}}
            raise
