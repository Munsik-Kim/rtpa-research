...
def source_snapshot(a):
    names=['diag_r2_benchmark.py','diag_r2_runtime.py','diag_r2_execution.py','codec_r2.py',
           ...
           'allocation_trace.py','diag_r2_analysis.py']
    return {n:sha(a.root/'src/rtpa_research'/n) for n in names if (a.root/'src/rtpa_research'/n).exists()}


def final_probe(a,budget):
    """Bounded post-wiring CAL parity, not another mask/codec selection."""
    import torch
    e=engine(a);ids=items(a,'CAL')[0]['input_ids'][:64];out=[];states=[]
    for method in ('R2_DIAG_REFERENCE','R2_DIAG'):
        c=new_cache(e,a,method);values=[]
        for token in ids:values.append(step(e,token,c,budget).cpu())
        out.append(torch.stack(values));states.append(hashes(c));del c,values;gc.collect()
    if not torch.equal(out[0],out[1]) or states[0]!=states[1]:raise ArithmeticError('FINAL_WIRING_PARITY_FAILURE')
    atomic(a.out/'final_probe.json',{'status':'PASS_BOUNDED_CAL','tokens_per_method':64,'physical_forwards':budget.calls,
        'logits_exact':True,'final_cache_tensors_exact':True,'every_intermediate_payload_compared':False,
        'sources':source_snapshot(a),'not_new_quality_evaluation':True})
...
def freeze(a,budget=None):
    ...
    for key,path in [('policy_sha256',a.out/'policy_r2.npz'),('selection_sha256',a.out/'codec_selection.json'),
                     ('TRAIN_panel_sha256',a.out/'train_panel.json'),('checker_sha256',a.root/'scripts/revalidate_diag_r2_inputs.py')]:
        if sha(path)!=revalidation[key]:raise ValueError('Revalidation binding changed:'+key)
    ...
    fp=read(a.out/'final_probe.json')
    if fp['status']!='PASS_BOUNDED_CAL' or fp['sources']!=source_snapshot(a):raise ValueError('Final source probe changed')
    ...
    def estimate():
        ...
        return {'quality_seconds':qual,'timing_seconds':timing,'memory_seconds':memory,'installed_API_smoke_reserved_seconds':50,
                'total_with20percent_margin_and_report_reserve':1.2*(qual+timing+memory+50)+reserve}
    ...
    package_paths += [a.root/'examples/diag_r2.py',a.root/'examples/diag_r2_model.py']
    ...
    for name in ('protocol.json','test_panel.json','train_panel.json','cal_panel.json','timing_panel.json',
                 ...
                 'final_cpu_validation.json','preTEST_wiring_revision.json','final_probe.json'):
...
def isolation():
    ...
    for line in result.stdout.splitlines():
        ...
        rows.append({'pid':pid,'process':Path(fields[1].strip()).name if len(fields)>1 else None})
...
def timing(a,budget):
    ...
    for block in range(-1,p['timing_measured_blocks']):
        ...
        for oi,label in enumerate(order):
            ...
            if block==-1:
                for t,token in enumerate(ids):context_step(e,token,c,budget,a,{'method':label,'block':block,'token':t})
                del c;gc.collect();continue
            gc.collect();sync();start=time.perf_counter()
            for t,token in enumerate(ids[:pref]):context_step(e,token,c,budget,a,{'method':label,'block':block,'token':t})
            sync();ps=time.perf_counter()-start;start=time.perf_counter()
            for t,token in enumerate(ids[pref:pref+dec],pref):context_step(e,token,c,budget,a,{'method':label,'block':block,'token':t})
            ...
            row={'block':block,'order':oi,'method':label,'prompt_tokens':pref,'decode_tokens':dec,
                 ...
                 'metric_scalar_logging_in_timed_region':False,'mandatory_guard_host_sync':True,'mandatory_codec_guards_retained':True,
...
def memory(a,budget):
    ...
    for t,token in enumerate(ids):
        context_step(e,token,c,budget,a,{'method':a.method,'token':t})
...
def measure_codec_allocations(codec,layout,z,outfile):
    ...
    for operation in ('encode','decode'):
        ...
        from .allocation_trace import summarize_allocations
        accounting=summarize_allocations(clean,output_ptrs)
...
def context_step(e,token,c,budget,a,context):
    """Normal forward unchanged; preserve first failing cache before unwinding."""
    try:return step(e,token,c,budget)
    except (FloatingPointError,ValueError) as exc:
        context={**context,'phase':a.phase,'reason':str(exc)}
        name=f'{a.phase}_{context.get("method","unknown")}_{time.time_ns()}'
        detail=cache_failure(c,a.out/'phase_failures'/f'{name}.npz',exception=exc)
        atomic(a.out/'phase_failures'/f'{name}.json',{**context,**detail})
        raise


def main(argv=None):
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--phase',choices=['conformance','final_probe','freeze','evaluate','timing','memory'],required=True)
