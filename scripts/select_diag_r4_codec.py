...
def verify_binding(root,baseline_run,candidate_run,rules):
    ...
    if bm['codec']!='LEGACY_P_PRE' or cm['codec']!='R4_OFFSET_RNE_V1':raise ValueError('CODEC_LABEL_MISMATCH')
    if base['manifest']['methods']['NATIVE']['codec']!='NATIVE' or cand['manifest']['methods']['NATIVE']['codec']!='NATIVE':
        raise ValueError('NATIVE_BASELINE_CODEC_MISMATCH')
