...
def analysis_source_binding(run,contract):
    """Resolve an explicit source-only revision without rewriting the old contract."""
    current=sha(__file__);original=contract['analysis_source_sha256']
    if current==original:return {'original_source_sha256':original,'executed_source_sha256':current,'revision_sha256':None}
    revision_path=run/'analysis_wiring_revision.json';revision=read(revision_path)
    require(revision['schema']=='R4_COST_ANALYSIS_WIRING_V2' and revision['GPU_work'] is False and revision['pre_cost_measurement'] is True,'ANALYSIS_WIRING_SCHEMA')
    require(revision['original_analysis_source_sha256']==original and revision['new_analysis_source_sha256']==current,'ANALYSIS_SOURCE_REVISION_BINDING')
    require(revision['cost_freeze_sha256']==contract['cost_freeze_sha256'] and revision['cost_wiring_revision_sha256']==contract['cost_wiring_revision_sha256'],'ANALYSIS_REVISION_COST_BINDING')
    require(revision['original_analysis_contract_sha256']==sha(run/'analysis_contract.json')==sha(run/'analysis_source_before_wiring/analysis_contract.json'),'ORIGINAL_ANALYSIS_CONTRACT_ARCHIVE')
    require(sha(run/'analysis_source_before_wiring/analyze_diag_r4_cost.py')==original,'ORIGINAL_ANALYZER_ARCHIVE')
    require(revision['bootstrap_draws_sha256']==contract['bootstrap_draws_sha256']==sha(run/'bootstrap_draws.npy'),'UNCHANGED_BOOTSTRAP_DRAWS')
    require(revision['native_storage_diagnostic_sha256']==sha(run/'analysis_native_storage_diagnostic.json'),'NATIVE_STORAGE_DIAGNOSTIC_BINDING')
    return {'original_source_sha256':original,'executed_source_sha256':current,'revision_sha256':sha(revision_path)}


def wire_analysis(root,run,evaluation_run):
    require(not list((run/'timing_attempts').glob('attempt_*.json')) and not list(run.glob('timing_schedule_*.json')) and
        not (run/'profile.json').exists() and not list((run/'memory').glob('*.json')) and not list((run/'failures').glob('*.json')),'ANALYSIS_WIRING_MUST_PRECEDE_ALL_COST_MEASUREMENTS')
    _,bound=binding(root,run,evaluation_run);contract=read(run/'analysis_contract.json')
    require(sha(run/'analysis_source_before_wiring/analyze_diag_r4_cost.py')==contract['analysis_source_sha256'],'ORIGINAL_ANALYZER_ARCHIVE')
    require(sha(run/'analysis_source_before_wiring/analysis_contract.json')==sha(run/'analysis_contract.json'),'ORIGINAL_ANALYSIS_CONTRACT_ARCHIVE')
    diagnostic=read(run/'analysis_native_storage_diagnostic.json')
    require(diagnostic['status']=='CONFIRMED_ANALYZER_SCHEMA_ERROR_BEFORE_COST' and diagnostic['CUDA_initialized'] is False,'STORAGE_DIAGNOSTIC')
    revision={'schema':'R4_COST_ANALYSIS_WIRING_V2','UTC':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),
        'GPU_work':False,'pre_cost_measurement':True,'original_analysis_contract_sha256':sha(run/'analysis_contract.json'),
        'original_analysis_source_sha256':contract['analysis_source_sha256'],'new_analysis_source_sha256':sha(__file__),
        'original_tests_sha256':sha(run/'analysis_source_before_wiring/test_diag_r4_cost_analysis.py'),
        'new_tests_sha256':sha(root/'tests/test_diag_r4_cost_analysis.py'),'bootstrap_draws_sha256':sha(run/'bootstrap_draws.npy'),
        'cost_freeze_sha256':bound['cost_freeze_sha256'],'cost_wiring_revision_sha256':bound['cost_wiring_revision_sha256'],
        'native_storage_diagnostic_sha256':sha(run/'analysis_native_storage_diagnostic.json'),
        'change':'Correct Native persistent cache descriptor from FP32 to BF16 (2 bytes), and explicitly resolve the archived analysis contract to this source revision.',
        'unchanged':['original analysis contract bytes','saved bootstrap draws','all statistical formulas, thresholds, counts and seeds',
            'GPU cost driver and cost schedule','model, codecs and masks','Native payload 9437184 bytes and 32768 bytes/head'],
        'supplemental_dependency_scope':'cache_utils.py hash and tiny CPU behavior were observed after the original execution freeze; it was not separately hash-bound by that original freeze.'}
    path=run/'analysis_wiring_revision.json'
    if path.exists():revision['UTC']=read(path)['UTC']
    save_once(path,revision);analysis_source_binding(run,contract)
    return {'status':'ANALYSIS_WIRING_V2_REGISTERED_BEFORE_COST','analysis_wiring_revision_sha256':sha(path)}
...
def memory_summary(run,bound):
    ...
    for method in ('NATIVE','B1','B4'):
        ...
        for tensor in row['actual_payload_tensor_dtype_numel'].values():
            ...
            if method=='NATIVE':require(tensor['dtype']=='torch.bfloat16','NATIVE_RECURRENT_STATE_DTYPE')
...
def analyze(root,run,evaluation_run,out,check_local_external=False):
    ...
    source_binding=analysis_source_binding(run,contract)
    expected['analysis_source_sha256']=source_binding['original_source_sha256']
    ...
    result={'status':timing['status'],'timing':timing,'attempts':retained,
        ...
        'binding':bound,'analysis_source_wiring':source_binding,'analysis_contract_sha256':sha(run/'analysis_contract.json'),'bootstrap_draws_sha256':sha(run/'bootstrap_draws.npy'),
    ...
    require(analysis_source_binding(run,contract)==source_binding and sha(run/'analysis_contract.json')==contract_hash and
...
def main():
    ...
    parser.add_argument('--phase',choices=['freeze','wire','analyze'],required=True)
    ...
    if args.phase=='freeze':result=register(args.root,args.run,args.evaluation_run)
    elif args.phase=='wire':result=wire_analysis(args.root,args.run,args.evaluation_run)
    else:result=analyze(args.root,args.run,args.evaluation_run,args.out or args.run/'analysis',args.check_local_external)
