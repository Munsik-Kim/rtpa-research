"""Selected-codec B0--B4 fitting; explicit factory, historical pilot unchanged."""
import argparse
import os
...
def main():
    ...
    cfgpath=a.root/'configs/diag_r4_selected_scores.json';cfg=read(cfgpath)
    sources=[Path(__file__),cfgpath,*[a.root/'src/rtpa_research'/n for n in
        ('calibration_r4.py','diag_r4_adjoint.py','operators.py','codec.py','layout.py','io.py',
         'diag_r2_benchmark.py','benchmark.py','codec_r4_rne.py','codec_r2.py',
         'diag_r2_runtime.py','runtime.py','factorized.py','reference_encoder.py',
         'codec_r2_optimized.py')],a.root/'tests/test_calibration_r4.py']
    ...
    if cfg['codec']=='R4_OFFSET_RNE_V1':
        codec=CodecR4RNE(a.device)
    elif cfg['codec']=='LEGACY_P_PRE':
        codec=Codec('P_PRE',a.device)
    else:raise ValueError('UNREGISTERED_SELECTED_CODEC')
    ...
    try:
        for row in files:
            ...
            tick=time.monotonic();result=response_r4(trace,codec,retain_fixture=keep)
        ...
        if len(completed)==len(files):
            ...
            atomic(a.out/'selection.json',{'status':'COMPLETE','policy_sha256':sha(a.out/'policy.npz'),
                ...
                'B4_entire_path_alias_legacy':cfg['codec']=='LEGACY_P_PRE' and all(x['mask_exact_legacy'] for x in overlaps if x['method']=='B4'),
