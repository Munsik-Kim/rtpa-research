...
def check(root):
    ...
    for path in manifests:
        ...
        for extra in stage.rglob('*'):
            if extra.is_file() and str(extra.relative_to(stage)) not in expected:
                errors.append({'stage': stage.name, 'path': str(extra.relative_to(stage)), 'reason': 'UNINVENTORIED_FILE'})
