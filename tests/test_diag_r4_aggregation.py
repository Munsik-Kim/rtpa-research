...
CHECK_SPEC = importlib.util.spec_from_file_location('check_r4', Path(__file__).resolve().parents[1] / 'scripts/check_diag_r4_public.py')
public_check = importlib.util.module_from_spec(CHECK_SPEC)
CHECK_SPEC.loader.exec_module(public_check)
...
class PublicBytecodeInventoryTests(unittest.TestCase):
    def fixture(self, root):
        source = root / 'historical_source/original.py'
        source.parent.mkdir(parents=True)
        source.write_text("raise AssertionError('historical source must not execute')\n")
        write_json(root / 'publication.json', {'scientific_status': 'TOY', 'files': [
            {'path': str(source.relative_to(root)), 'sha256': a.sha(source), 'bytes': source.stat().st_size}]})
        return source

    def test_installer_bytecode_for_inventoried_source_is_reported_not_evidence(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            source = self.fixture(root)
            for optimization in ('', '1', '2'):
                cache = importlib.util.cache_from_source(str(source), optimization=optimization)
                py_compile.compile(str(source), cfile=cache, doraise=True, optimize=int(optimization or 0))
            result = public_check.check(root)
            self.assertEqual(result['status'], 'PASS')
            excluded = result['derived_bytecode_excluded_from_evidence']
            self.assertEqual(len(excluded), 3)
            self.assertEqual(len(result['stages'][0]['files']), 1)
            for row in excluded:
                self.assertEqual(row['inventoried_source'], 'historical_source/original.py')
                self.assertEqual(row['status'], 'DERIVED_BYTECODE_EXCLUDED_FROM_EVIDENCE')
                self.assertFalse(row['bytecode_hash_verified'])
                self.assertFalse(row['bytecode_loaded_or_executed'])

    def test_unknown_files_and_nonconventional_caches_are_still_rejected(self):
        extras = ('historical_source/unknown.py', 'historical_source/original.pyc',
                  'historical_source/__pycache__/unknown.' + sys.implementation.cache_tag + '.pyc',
                  'historical_source/__pycache__/original.unrecognized.pyc',
                  'historical_source/__pycache__/original.' + sys.implementation.cache_tag + '.opt-3.pyc',
                  'historical_source/__pycache__/extra.json')
        for name in extras:
            with self.subTest(path=name), tempfile.TemporaryDirectory() as temp:
                ...
                self.assertEqual(result['errors'], [{'stage': root.name, 'path': name, 'reason': 'UNINVENTORIED_FILE'}])

    def test_cache_does_not_hide_changed_or_missing_inventoried_source(self):
        for missing in (False, True):
            with self.subTest(missing=missing), tempfile.TemporaryDirectory() as temp:
                ...
                result = public_check.check(root)
                self.assertEqual(result['status'], 'FAIL')
                self.assertIn('MISSING' if missing else 'HASH_OR_SIZE_MISMATCH',
                              [row['reason'] for row in result['errors']])

    def test_symlink_at_conventional_cache_path_is_not_excluded(self):
        for kind in ('file', 'dangling_file', 'directory'):
            with self.subTest(kind=kind), tempfile.TemporaryDirectory() as temp:
                root = Path(temp) / 'stage'
                source = self.fixture(root)
                cache = Path(importlib.util.cache_from_source(str(source)))
                target = Path(temp) / 'not_evidence'
                if kind == 'directory':
                    target.mkdir()
                    (target / cache.name).write_bytes(b'not an installer-owned regular cache')
                    cache.parent.symlink_to(target, target_is_directory=True)
                else:
                    if kind == 'file':
                        target.write_bytes(b'not an installer-owned regular cache')
                    cache.parent.mkdir()
                    cache.symlink_to(target)
                result = public_check.check(root)
                self.assertEqual(result['status'], 'FAIL')
                self.assertEqual(result['derived_bytecode_excluded_from_evidence'], [])
                self.assertEqual(result['errors'][0]['reason'], 'UNINVENTORIED_FILE')
