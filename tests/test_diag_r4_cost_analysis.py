...
def test_memory_separates_80byte_counter_and_rejects_reused_pid(tmp_path,complete):
    ...
    for index,method in enumerate(('NATIVE','B1','B4')):
        ...
        row.update(status='COMPLETE',method=method,pid=100+index,fresh_process=True,physical_forwards=544,prefix=512,decode=32,
            ...
            actual_payload_tensor_dtype_numel={'payload':{'dtype':'torch.bfloat16' if method=='NATIVE' else 'torch.uint8','numel':payload//2 if method=='NATIVE' else payload,'bytes':payload}},
    ...
    row['pid']=102;path.write_text(json.dumps(row))
    native_path=tmp_path/'memory/NATIVE.json';native=analysis.read(native_path)
    native['actual_payload_tensor_dtype_numel']['payload'].update(dtype='torch.float32',numel=9437184//4)
    native_path.write_text(json.dumps(native))
    with pytest.raises(ValueError,match='NATIVE_RECURRENT_STATE_DTYPE'):analysis.memory_summary(tmp_path,bound)


def test_explicit_source_revision_preserves_old_contract_and_rejects_archive_mutation(tmp_path):
    archive=tmp_path/'analysis_source_before_wiring';archive.mkdir()
    source=archive/'analyze_diag_r4_cost.py';source.write_text('original source with identified schema error\n')
    np.save(tmp_path/'bootstrap_draws.npy',np.zeros((2000,8),dtype=np.int64),allow_pickle=False)
    analysis.save_once(tmp_path/'analysis_native_storage_diagnostic.json',{'scope':'synthetic fixture'})
    contract={'analysis_source_sha256':analysis.sha(source),'bootstrap_draws_sha256':analysis.sha(tmp_path/'bootstrap_draws.npy'),
        'cost_freeze_sha256':'old-cost','cost_wiring_revision_sha256':'cost-wire'}
    analysis.save_once(tmp_path/'analysis_contract.json',contract)
    analysis.save_once(archive/'analysis_contract.json',contract)
    before=analysis.sha(tmp_path/'analysis_contract.json')
    revision={'schema':'R4_COST_ANALYSIS_WIRING_V2','GPU_work':False,'pre_cost_measurement':True,
        'original_analysis_source_sha256':analysis.sha(source),'new_analysis_source_sha256':analysis.sha(analysis.__file__),
        'cost_freeze_sha256':'old-cost','cost_wiring_revision_sha256':'cost-wire','original_analysis_contract_sha256':before,
        'bootstrap_draws_sha256':contract['bootstrap_draws_sha256'],
        'native_storage_diagnostic_sha256':analysis.sha(tmp_path/'analysis_native_storage_diagnostic.json')}
    analysis.save_once(tmp_path/'analysis_wiring_revision.json',revision)
    result=analysis.analysis_source_binding(tmp_path,contract)
    assert result['original_source_sha256']!=result['executed_source_sha256']
    assert result['revision_sha256']==analysis.sha(tmp_path/'analysis_wiring_revision.json')
    assert analysis.sha(tmp_path/'analysis_contract.json')==before
    source.write_text('changed archive')
    with pytest.raises(ValueError,match='ORIGINAL_ANALYZER_ARCHIVE'):analysis.analysis_source_binding(tmp_path,contract)
