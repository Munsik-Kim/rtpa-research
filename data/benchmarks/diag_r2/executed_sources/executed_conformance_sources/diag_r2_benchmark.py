...
def step(e,token,c,b):
    ...
    if not bool(torch.isfinite(logits).all()):
        exc=FloatingPointError('NONFINITE_LOGITS')
        exc.snapshot_logits=logits.detach().float().cpu()
        raise exc
    return logits
...
def cache_failure(c,path,logits=None,exception=None):
    import torch
    rows={}; arrays={}
    if logits is None and exception is not None:logits=getattr(exception,'snapshot_logits',None)
    def retain(value,key):
        if isinstance(value,torch.Tensor):
            tensor=value.detach().cpu().contiguous()
            if tensor.dtype==torch.bfloat16:tensor=tensor.float()
            arrays[key]=tensor.numpy()
            return {'tensor_key':key,'dtype':str(value.dtype),'shape':list(value.shape)}
        if isinstance(value,dict):return {k:retain(v,key+'/'+k) for k,v in value.items()}
        return value
    for i,layer in enumerate(c.layers):
        failure=getattr(layer,'last_failure',None)
        if failure:
            rows[str(i)]=retain(failure,str(i))
...
def capture(a,budget):
    ...
    for item in items(a,'TRAIN'):
        ...
        if done.exists():
            rr=read(done)
            if rr['input_sha256']!=item['token_sha256'] or rr['profile']!=profile:raise ValueError('capture identity mismatch')
            if not all(sha(a.out/r['path'])==r['sha256'] for r in rr['files']):raise ValueError('capture receipt mismatch')
            if rr.get('source_binding')!=phase_binding(a):raise ValueError('capture source binding missing or changed; explicit preTEST revalidation required')
            continue
        ...
        atomic(done,{'status':'COMPLETE','files':files,'physical_forwards':budget.calls-start,'seconds':time.monotonic()-tick,
                     'sequence_id':item['id'],'input_sha256':item['token_sha256'],'profile':profile,'source_binding':phase_binding(a)})
...
def fit(a,budget):
    ...
    capture_files={}
    for item in items(a,'TRAIN'):
        rr=read(a.out/'capture'/f'{item["id"]}.json')
        if rr['input_sha256']!=item['token_sha256'] or rr['profile']!=profile:raise ValueError('capture identity mismatch')
        for record in rr['files']:
            if sha(a.out/record['path'])!=record['sha256']:raise ValueError('capture numerical bytes changed')
            capture_files[record['path']]=record['sha256']
    for l in layers:
        path=a.out/'fit'/f'layer{l}.npz';done=path.with_suffix('.json')
        if done.exists():
            rr=read(done);assert sha(path)==rr['sha256']
            if rr.get('source_binding')!=phase_binding(a):raise ValueError('fit source binding missing or changed; explicit preTEST revalidation required')
            if rr.get('capture_files')!=capture_files:raise ValueError('fit capture binding changed')
        ...
        rr={'layer':l,'sha256':sha(path),'seconds':time.monotonic()-tick,'peak_allocated_bytes':torch.cuda.max_memory_allocated(),
            ...
            'source_binding':phase_binding(a),'capture_files':capture_files}
...
def phase_binding(a):
    return {'source':{n:sha(Path(__file__).with_name(n)) for n in
                     ('diag_r2_benchmark.py','calibration_r2.py','calibration.py','codec_r2.py','diag_r2_runtime.py','runtime.py')},
            'TRAIN_sha256':sha(a.out/'train_panel.json'),'selection_sha256':sha(a.out/'codec_selection.json')}
